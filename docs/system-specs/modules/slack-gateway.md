...
alone. Before `_channel_transports_started` the applier retains the change
instead of restarting, because the boot loop starts every channel from the hoist
and a restart there would race it; the boot loop replays the retained change
right after `start_channels`. That replay covers boot keys only, so live fields
...
loop first re-hoists every bootable channel from the watcher's CURRENT snapshot
(`_adopt_channel_sections_from_watcher`), replacing the channel's section on
`self._cfg` and re-running its `_hoist_<channel>`, so the first transport is
built from the document as it is, not as it was at construction. A snapshot whose
